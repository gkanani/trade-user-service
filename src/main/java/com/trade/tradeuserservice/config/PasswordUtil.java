package com.trade.tradeuserservice.config;
...
public class PasswordUtil {
    ...
    private static final byte[] INIT_VECTOR = "Prof.G.H.Gardy#1".getBytes();

    private static final String LENGTH_ERROR = "Invalid password length found. Only >= 8 length is allowed.";
    private static final String SALT_SECRET_KEY_LENGTH_ERROR = "A string with healthy length is required to generate tasty salt.";
    ...
    public static String encrypt(String str, String secretKey, String salt) {
        try {
            IvParameterSpec ivParamSpec = new IvParameterSpec(INIT_VECTOR);
    ...
    public static String generateSecretKeyFromString(String str) {
        if (null != str && !str.trim().isEmpty()) {
            try {
                return digestMessageWithMilliSecond(str);
            } catch (NoSuchAlgorithmException e) {
                logger.error("Something went wrong while generating secret key, please try again.");
            }
        } else {
            throw new IllegalArgumentException(SALT_SECRET_KEY_LENGTH_ERROR);
        }
        return EMPTY;
    }
    ...
    public static String generateSaltFromString(String str) {
        if (null != str && !str.trim().isEmpty()) {
            try {
                return digestMessageWithMilliSecond(str);
            } catch (NoSuchAlgorithmException e) {
    ...
    /*Extra utility methods for processing of password/salt/key.*/

    @SuppressWarnings("unused")
    private boolean guessOddEven(int length) {
        return (length % 2) == 0;
    }
    ...
    public static String generateRandomString(int length) {
        if (FIXED_LENGTH > length) {
            ...
            writePassword(MIXED_CHARS, length, randomStringBuilder);
            return shuffleChars(randomStringBuilder.toString());
        }
    }

    private static String digestMessageWithMilliSecond(String str) throws NoSuchAlgorithmException {
        if (null != str && !str.trim().isEmpty()) {
            return digestMessage(str + "_" + System.currentTimeMillis());
        } else {
            throw new IllegalArgumentException(SALT_SECRET_KEY_LENGTH_ERROR);
        }
    }
    ...
    private static String shuffleChars(String pass) {
        List<String> genratedChars = Arrays.asList(pass.split(""));
        Collections.shuffle(genratedChars);
        return genratedChars.stream().collect(Collectors.joining());
    }
